...
- **Live public telemetry (when deployed):** `magic-trick.html` **fetches** **`GET /api/magic-trick-telemetry`** (Vercel Node serverless), which aggregates **real NOAA SWPC JSON** (planetary K 1-min — prior vs latest samples, GOES soft X-ray, RTSW L1 wind/mag) via **`lib/observatory-public-evidence.mjs`**, plus **published catalog** lines: **H I rest 1420.405751 MHz**, **Crab Pulsar PSR B0531+21** period (catalog ephemeris class — not live timing from this site). **No mock Kp.** KiwiSDR / grid / EGS “1.0000” are **not** fabricated: the API states honest limits (no global KiwiSDR aggregate API; no EGS sensor in NOAA JSON).
- **Static file, dynamic data:** the HTML shell is static; the **table body** updates from the API response on each load. If the API is unavailable (e.g. `file://` or no server), the UI shows an error and points to **`vercel dev`** / **`npm run ping:public`**. The former in-browser EXECUTE flow (369 Hz XOR latch, **180 LOCKED** UI) remains **removed**.
