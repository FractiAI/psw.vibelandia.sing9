 ...
 * 2) Local file (developer) or OS temp file (serverless when HLINE_MEMORY_FILE unset)
 *
 * NSPFRNP -> infinity 9
 */
...
const DEFAULT_REPLICA_NODES = ['sol-core-1', 'europa-node-1', 'ganymede-node-1', 'callisto-node-1'];
...
function normalizeStorePath() {
  const env = process.env.HLINE_MEMORY_FILE;
  if (env && String(env).trim()) {
    return path.resolve(process.cwd(), String(env));
  }
  const isServerless =
  ...
  if (isServerless) {
    return path.join(os.tmpdir(), 'hline-memory-store.json');
  }
  return path.resolve(process.cwd(), 'data/hline-memory-store.json');
}
...
function configuredReplicaNodes() {
  const raw = process.env.HLINE_REPLICA_NODES;
  if (!raw) return DEFAULT_REPLICA_NODES;
  return raw.split(',').map((s) => s.trim()).filter(Boolean);
}

function replicationFactorForTier(tier) {
  if (tier === 'io') return 2;
  if (tier === 'europa') return 3;
  if (tier === 'ganymede') return 3;
  return 4;
}

function selectReplicaSet(tier) {
  const nodes = configuredReplicaNodes();
  const rf = Math.min(replicationFactorForTier(tier), nodes.length);
  return nodes.slice(0, rf);
}
...
export async function writeHydrogenLineMemory({
  ...
  const record = {
    ...
    tier,
    replication: {
      factor: replicationFactorForTier(tier),
      replica_set: selectReplicaSet(tier),
    },
...
export function getHydrogenLineMemoryConfigSummary() {
  ...
  if (gh.enabled) {
    return {
      ...
      strict_no_legacy_mode: strictNoLegacy,
    };
  }
  return {
